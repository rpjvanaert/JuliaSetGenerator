...
public class JuliaSetLogic {
    ...
    public void initMandleBrot(){
        ...
        for (float r = rMin + stepSize; r < rMax; r += stepSize){
            for (float i = iMin + stepSize; i < iMax; i += stepSize){
                this.iterations[indexR][indexI] = getMandleCoor(r, i);
                indexI++;
            }
            indexI = 0;
            indexR++;
    ...
    private int getMandleCoor(float real, float imag){
        return this.juliaIterations(0.0f, 0.0f, real, imag);
    }
